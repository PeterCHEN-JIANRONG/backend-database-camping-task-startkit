...
select u."name" as "教練名稱", c.experience_years as "經驗年數", s."name" as "專長名稱" from "COACH_LINK_SKILL" cls
...
select s."name" as "專長名稱", count(*) as coach_total from "COACH_LINK_SKILL" cls
join "SKILL" s on s.id = cls.skill_id
group by s.id
...
select cpa.name as "組合包方案名稱", count(*) as "銷售數量" from "CREDIT_PURCHASE" cp
join "CREDIT_PACKAGE" cpa on cpa.id = cp.credit_package_id
where cp.created_at between '2024-12-01 00:00:00' and '2024-12-31 23:59:59'
group by cpa.id;
